...
contract Library {
    ...
    function getBook(uint256 index) public view returns (string memory, bool) {
    ...
    function updateBookTitle(uint256 index, string memory newTitle) public {
        require(index < books.length, "Index out of bounds");
        books[index].title = newTitle;
    }

    // Function to toggle the isFiction status of a book
    function toggleIsFiction(uint256 index) public {
        require(index < books.length, "Index out of bounds");
