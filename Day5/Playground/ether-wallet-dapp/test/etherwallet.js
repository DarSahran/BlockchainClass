   ...
   describe('EtherWallet', function () {
    ...
    describe('Deposit', function () {
       it('Should deposit Ether to the contract', async function () {
        ...
        const balance = await ethers.provider.getBalance(etherWallet)
    ...
    describe('Withdraw', function () {
       it('Should withdraw ether from the contract with zero ETH(Not a very useful test)', async function () {
        ...
        const tx = await etherWallet
           .connect(owner)
           .withdraw(owner.address, ethers.parseEther('0'))
        await tx.wait()
       ...
       it('Should withdraw ether from the contract with non-zero ETH', async function () {
        ...
        let balance = await ethers.provider.getBalance(etherWallet)
        expect(balance.toString()).to.equal(ethers.parseEther('1'))
        ...
        balance = await ethers.provider.getBalance(etherWallet)
        expect(balance.toString()).to.equal(ethers.parseEther('0'))
       })
       ...
       it('Should revert the tx when withdraw is called by someone other than the owner', async function () {
        ...
        await expect(
           etherWallet
            ...
            .withdraw(otherAccount.address, ethers.parseEther('0'))
